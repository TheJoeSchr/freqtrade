...
class Exchange:
    ...
    async def _async_get_trade_history_id(self, pair: str,
        ...
        while True:
            trade = await self._async_fetch_trades(pair,
            ...
            if trade:
                ...
                id = trade[-1][1]
                time = trade[-1][0]
                # stop if last id is reached
                # or trade time is later than until
                if (from_id == id if stop_on_from_id else False) or (time > until if until else False):
                    logger.debug(f"Stopping because from_id did not change: {from_id == id}. "
                                 f"or reached {time} > {until}")
